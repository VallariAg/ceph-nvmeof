...
class GatewayService(pb2_grpc.GatewayServicer):
    ...
    def list_namespaces_io_stats(self, request, context=None):
        ...
        failure_prefix = f"Failure listing IO stats for namespaces {request.nsid} " \
                         f"on {request.subsystem_nqn}"
        peer_msg = self.get_peer_message(context)
        self.logger.info(f"Received request to list IO stats for namespaces {request.nsid} on "
