...
class GatewayService(pb2_grpc.GatewayServicer):
    ...
    def gw_refresh_network_safe(self, request, context):
        ...
        self.logger.info(f"Received request to refresh network listeners for subsystem "
        ...
        try:
            ...
            for mask in subsys_entry.network_mask:
                subnet_ips.update(nics.get_ips_in_subnet(mask))
            ...
            removed = []
            if to_add:
                err_msg, added = self.add_listeners(request.subsystem_nqn, list(to_add),
            ...
            if to_remove:
                err_msg, removed = self.del_listeners(request.subsystem_nqn, list(to_remove),
                                                      subsys_entry.secure_listeners,
                                                      subsys_entry.port)
                if err_msg:
                    if final_err_msg:
