...
class GatewayService(pb2_grpc.GatewayServicer):
    ...
    def create_subsystem_safe(self, request, context):
        ...
        if request.default_listeners:
            try:
                ...
                elif req_status != 0 and not context:
                    self.logger.info("Problem when creating auto listeners.")
                else:
    ...
    def create_auto_listeners(self, request, context):
        ...
        if request.default_listeners and config_default_listeners:
            ip_addr = self.config.get("gateway", "addr")
            adrfam = f'ipv{ip_address(ip_addr).version}'
