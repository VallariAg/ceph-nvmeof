        ...
        if len(allow_listeners) == 0:
            for host in hosts:
                if host["host_nqn"] == '*' or host["host_nqn"] == hostnqn:
                    ...
                    self.logger.info(f"VALLARI_DEBUG_5.1 looking!: {subsystem_nqn}")
                    if subsystem_nqn in nvmemon_listeners:
                        self.logger.info(f"VALLARI_DEBUG_5 FOUND!: {subsystem_nqn}")
